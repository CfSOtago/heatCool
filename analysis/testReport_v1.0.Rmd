...
# hpDT_orig <- data.table::fread(dataFile)
...
The next table gives a summary of the heat pump data after limited processing (no cleaning). The exact values here will depend on the data source used.
...
Table \@ref(tab:hpCircuitLabels) gives a list of all circuit labels seen - as a check on what we think they are measuring.
...
Figure \@ref(fig:simpleHistHPno0) shows same plot but only for power > 0 W (i.e. on standby or heating) and only for the circuits we want to include. Does this look right?

```{r simpleHistHPno0, fig.cap="Histogram mean hourly power demand for Heat Pumps by season and location"}

dt <- hpDT[powerW > 0 & includeCircuit == 1]
...
# Test relationship between temperature and heat pump demand (all data)

So now we think we've linked the data correctly, let's do some exploratory analysis. In this section we keep all the data, including the observations where power < 40 for the circuits we wish to include.

First we just plot all the hourly data for demand vs mean temperature (Figure \@ref(fig:tempDemandScatterAll)).

```{r tempDemandScatterAll, fig.cap="Plots of temperature vs demand by season and location"}
dt <- linkedDT[includeCircuit == 1 & !is.na(meanTemp)]

t <- dt[, .(meanW = mean(powerW),
            minW = min(powerW),
            maxW = max(powerW),
            nObs = .N,
...
hpCap <- paste0("GREEN Grid Heat Pump circuits, n households = ", uniqueN(dt$id),
                    "\n Time period: ", min(lubridate::date(dt$r_dateTimeLocal)), 
                    " to ",
                    max(lubridate::date(dt$r_dateTimeLocal)),
                "\n n circuits: ", uniqueN(dt$circuit)
                )
...
ggplot2::ggplot(dt, aes(x = meanTemp, y = powerW/1000, colour = id)) +
  geom_point() +
...
```{r tempDemandBoxSeasonAll, fig.cap="Box plots of temperature vs demand by season and location"}
ggplot2::ggplot(dt, mapping = aes(x = meanTemp, y = powerW/1000)) + 
...
# Test relationship between temperature and heat pump demand (filtered data)

Notwithstanding the previous argument re data exclusion, in this section we test Emily's suggestion to only keep heat pump data where power > 40 for the circuits we wish to include.

First we just plot all the hourly data for demand vs mean temperature (Figure \@ref(fig:tempDemandScatterFiltered)).

```{r tempDemandScatterFiltered, fig.cap="Plots of temperature vs demand by season and location"}
...
```{r tempDemandBoxSeasonFiltered, fig.cap="Box plots of temperature vs demand by season and location"}
ggplot2::ggplot(dt, mapping = aes(x = meanTemp, y = powerW/1000)) + 
  geom_boxplot(aes(cut_width(meanTemp,
                             width = 2, # change the width to get 'wider' bins
                             boundary = 0)))  + 
...
Figure \@ref(fig:tempDemandBoxSeasonFiltered) shows binned box plots of power against demand. As we would expect, the plot suggests more clearly that there is a cooling effect for households in summer when we remove 0 and power < 40 W values.
