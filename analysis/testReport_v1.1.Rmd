...
ggplot2::ggplot(ejFilteredDT, mapping = aes(x = meanTemp, y = powerW/1000)) + 
  ...
  labs(caption = combinedCaption,
       x = "Mean temperature",
       y = "Mean half-hourly kW") +
  geom_smooth(modelDT, mapping = aes(temp, meanPowerWFit/1000),colour = 'red') + # mean fit
  geom_smooth(modelDT, mapping = aes(temp, medianPowerWFit/1000),colour = 'blue') # median fit
...
This gives the fit for all the houses. 
``` {r summaryejFilteredDTRangeLM}
summary(lm((ejFilteredDTRange$powerW)/1000~poly(ejFilteredDTRange$meanTemp, 2)))
```
...
Say that the model fits - now what?
